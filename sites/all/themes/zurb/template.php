...
function zurb_links__topbar_main_menu($variables) {
  ...
  $output = _zurb_links($links);
  $variables['attributes']['class'][] = 'left';

  return '<ul' . drupal_attributes($variables['attributes']) . '>' . $output . '</ul>';
}
function _zurb_links($links) {
  $output = '';

  foreach (element_children($links) as $key) {
    $output .= _zurb_render_link($links[$key]);
  }
  return $output;
}
  ...
  if (!empty($link['#href'])) {
    ...
    if (!empty($link['#below'])) {
      ...
      foreach ($link['#below'] as $sub_link) {
        if (!empty($sub_link['#below'])) {
          $sub_menu .= _zurb_render_link_p3($sub_link);
        }
        else {
          $sub_menu .= _zurb_render_link($sub_link);
        }
      }

      $output .= '<div class="dropbox-bg"><div class="dropbox"><ul class="idTabs hide_mobile1">' . $sub_menu . '</ul></div></div>';
    }
...
function _zurb_render_link_p3($link) {
  $output = '';
  ...
  if (!empty($link['#href'])) {
    ...
    if (!empty($link['#below'])) {
      ...
      $output .= '<div class="article-list"><ul>' . $sub_menu . '</ul></div>';
    }
